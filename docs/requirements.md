...
**Business-review template and Power BI link (post-brief)** — in September 2026 NHN replaced the review template's wording (the Word file at the repo root) and asked for an automatic link to each service's Power BI finance report. The link is stored on the service (`powerbi`, optional `powerbi-tekst`), filled in from suggestions drawn from each service's latest review on **Tjenesteeiere**, and appended to every review the form creates. The review ToDo compares against the seeded text, link included. See Phase 11 in [plan.md](plan.md).
