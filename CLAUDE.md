...
- **Run `npm test` after touching plugin code** (~9s, no deps beyond TiddlyWiki; `npm test -- <pattern>` is sub-second). Filters fail *silently* — a renamed projection exports a blank column rather than erroring — so the suite is the only thing that notices. Add cases with the behaviour you change; see [docs/testing.md](docs/testing.md).
